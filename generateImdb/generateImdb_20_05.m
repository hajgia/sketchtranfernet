...
%依次对各个类别的图片进行处理
for filenum= 203:252    
    files=dir(fullfile(dir_name,dirOutput(filenum).name,'*_0.png'));   
    fileNames2={files.name}';   %fileNames2存储的是当前类的所有后缀为*_0.png的文件的名字
    num=size(files,1);
    trainNum=num*3/4;
    testNum=num/4;
...
%存储imdb为mat格式
dir='E:\刘杨';
savefile=[dir,'\','Data','\',num2str(rows),'_20_05','.mat'];
