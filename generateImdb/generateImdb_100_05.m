...
%依次对各个类别的图片进行处理
for filenum= 203:252    
    files=dir(fullfile(dir_name,dirOutput(filenum).name,'*_4.png'));   
    fileNames2={files.name}';   %fileNames2存储的是当前类的所有后缀为*_4.png的文件的名字
    num=size(files,1);
    trainNum=num*3/4;
    testNum=num/4;
    ...
    for i=1:num
        ...
        labels(1,(filenum-203)*80+i)=filenum-2;
           
        if i>trainNum
            set(1,(filenum-203)*80+i)=3;
        end
    end    
    flag=flag+1 
end
...
%存储imdb为mat格式
dir='E:\刘杨';
savefile=[dir,'\','Data','\',num2str(rows),'_100_05','.mat'];
