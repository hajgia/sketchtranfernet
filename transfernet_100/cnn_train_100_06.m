function [net, info] = cnn_train_100_06(net, imdb1, imdb2, imdb3, imdb4, imdb5, getBatch, varargin)
...
for epoch=1:opts.numEpochs
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    res1 = vl_simplenn_100_06_16layer(net, im, [], res1, ...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
    res1(16).x = reshape(res1(16).x , [size(res1(16).x,3) , size(res1(16).x,4)]) ;
    ...
    res2 = [];
    res2 = vl_simplenn_100_06_16layer(net, testim, [], res2, ...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
    res2(16).x = reshape(res2(16).x , [size(res2(16).x,3) , size(res2(16).x,4)]) ;
    feature_error = sum(sum((res1(16).x-res2(16).x).^2)) ;
     
    res = vl_simplenn_100_06(net, im, one, res, feature_error, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
  ...
  for t=1:opts.batchSize:numel(val)
    ...
    net.layers{end}.class = labels ;
    res = vl_simplenn_100_06(net, im, [], res, [],...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
