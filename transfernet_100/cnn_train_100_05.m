function [net, info] = cnn_train_100_05(net, imdb1, imdb2, imdb3, imdb4, imdb5, getBatch, varargin)
...
if isempty(opts.train), opts.train = cat(2,find(imdb1.imdb.images.set==1),find(imdb2.imdb.images.set==1)+4000,find(imdb3.imdb.images.set==1)+8000,find(imdb4.imdb.images.set==1)+12000,find(imdb5.imdb.images.set==1)+16000) ; end
if isempty(opts.val), opts.val = cat(2,find(imdb1.imdb.images.set==3),find(imdb2.imdb.images.set==3)+4000,find(imdb3.imdb.images.set==3)+8000,find(imdb4.imdb.images.set==3)+12000,find(imdb5.imdb.images.set==3)+16000) ; end
if isnan(opts.train), opts.train = [] ; end

for i=1:numel(net.layers)
  if ~strcmp(net.layers{i}.type,'conv') || i==1 || i==4 || i==7 || i==9 || i==11 || i==14, continue; end
...
for epoch=1:opts.numEpochs
  ...
  if prevLr ~= lr
    ...
    for l=1:numel(net.layers)
      if ~strcmp(net.layers{l}.type, 'conv') || l==1 || l==4 || l==7 || l==9 || l==11 || l==14 , continue ; end
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    res1 = vl_simplenn_100_05_16layer(net, im, [], res1, ...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
    res1(20).x = reshape(res1(20).x , [size(res1(20).x,3) , size(res1(20).x,4)]) ;
    
    for x = 1:100
        y(x) = labels(x)*80-floor(rand()*20) ;
    end
    
    [testim,labels2] = getBatch(imdb1,imdb2,imdb3,imdb4,imdb5,y) ;
    testim = single(testim);
    res2 = [];
    res2 = vl_simplenn_100_05_16layer(net, testim, [], res2, ...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
    res2(20).x = reshape(res2(20).x , [size(res2(20).x,3) , size(res2(20).x,4)]) ;
    feature_error = (sum(sum(abs(res1(20).x-res2(20).x)))) ;
     
    res = vl_simplenn_100_05(net, im, one, res, feature_error, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
  ...
  for t=1:opts.batchSize:numel(val)
    ...
    net.layers{end}.class = labels ;
    res = vl_simplenn_100_05(net, im, [], res, [],...
      'disableDropout', true, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
