function [net, info] = transfernet_100_03(varargin)
...
net.layers{end+1} = struct('type', 'conv', ...                  %1
                           'filters', 0.01 * randn(15,15,3,3,'single'),...
                           'biases', 0.01*ones(1,3,'single'), ...
                           'stride', 1, ...
                           'pad', 0, ...
...
for x = 1:15
    net.layers{end+1} = Layers{x};
end
net.layers{end+1} = struct('type', 'conv', ...                  %18
                           ...
                           'filtersWeightDecay', 1, ...
                           'biasesWeightDecay', 0) ;
net.layers{end+1} = struct('type', 'relu') ;     %19
net.layers{end+1} = struct('type', 'dropout', ...    %20
                           'rate', 0.5) ;
net.layers{end+1} = struct('type', 'conv', ...       %21
                           ...
                           'filtersWeightDecay', 1, ...
                           'biasesWeightDecay', 0) ;
net.layers{end+1} = struct('type', 'softmaxloss') ;     %22
...
[net, info] = cnn_train_100_03(net, imdb1, imdb2, imdb3, imdb4, imdb5, @getBatch, ...
    opts.train) ;
